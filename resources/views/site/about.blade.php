...
@section('title', ($siteSettings['about_banner_title'] ?? 'About Us') . ' | ' . ($siteSettings['site_name'] ?? 'Global Manpower Overseas Ltd.'))
...
<div class="bg-slate-950 text-white py-16 border-b border-slate-800">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <span class="px-3 py-1 rounded-full text-xs font-bold bg-blue-500/20 text-blue-400 border border-blue-500/30 uppercase tracking-wider mb-4 inline-block">
            Govt. BMET License: {{ $siteSettings['bmet_license_no'] }}
        </span>
        @endif
        <h1 class="text-3xl lg:text-4xl font-extrabold">{{ $siteSettings['about_banner_title'] ?? ('About ' . ($siteSettings['site_name'] ?? 'Global Manpower Overseas Ltd.')) }}</h1>
        <p class="text-slate-400 mt-2 max-w-2xl text-base">{{ $siteSettings['about_banner_subtitle'] ?? 'Over 18 years of pioneering excellence in overseas human resource recruitment and ethical labor migration.' }}</p>
    </div>
</div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    <div class="grid lg:grid-cols-12 gap-12 items-start">
        <div class="lg:col-span-7 space-y-6">
            ...
            <div class="grid sm:grid-cols-2 gap-6">
                <div class="bg-blue-50/50 border border-blue-200 rounded-2xl p-6">
                    ...
                    <h3 class="text-lg font-bold text-slate-900 mb-2">{{ $siteSettings['about_mission_title'] ?? 'Our Mission' }}</h3>
                    <p class="text-sm text-slate-600 leading-relaxed">{{ $siteSettings['about_mission_statement'] ?? 'To empower Bangladeshi workforce with dignified, legal overseas employment while providing top-tier labor solutions to global employers.' }}</p>
                </div>
                ...
                <div class="bg-indigo-50/50 border border-indigo-200 rounded-2xl p-6">
                    ...
                    <h3 class="text-lg font-bold text-slate-900 mb-2">{{ $siteSettings['about_vision_title'] ?? 'Our Vision' }}</h3>
                    <p class="text-sm text-slate-600 leading-relaxed">{{ $siteSettings['about_vision_statement'] ?? 'To be the most ethical, transparent, and preferred overseas manpower consultancy in South Asia.' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-5 space-y-6">
            <div class="bg-slate-900 text-white rounded-2xl p-8 border border-slate-800 shadow-lg">
                <h3 class="text-xl font-bold mb-6 text-white border-b border-slate-800 pb-4">{{ $siteSettings['about_accreditation_title'] ?? 'Government Accreditation' }}</h3>
                <ul class="space-y-4 text-sm text-slate-300">
                    ...
                    <li class="flex items-start gap-3">
                        ...
                        <span><strong>BMET License:</strong> {{ $siteSettings['bmet_license_no'] }}</span>
                    </li>
                    @endif
                    @if(!empty($siteSettings['about_accreditation_1']))
                    <li class="flex items-start gap-3">
                        <svg class="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        <span>{{ $siteSettings['about_accreditation_1'] }}</span>
                    </li>
                    @endif
                    @if(!empty($siteSettings['about_accreditation_2']))
                    <li class="flex items-start gap-3">
                        <svg class="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        <span>{{ $siteSettings['about_accreditation_2'] }}</span>
                    </li>
                    @endif
                    @if(!empty($siteSettings['about_accreditation_3']))
                    <li class="flex items-start gap-3">
                        <svg class="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        <span>{{ $siteSettings['about_accreditation_3'] }}</span>
                    </li>
                    @endif
                    @if(!empty($siteSettings['about_accreditation_4']))
                    <li class="flex items-start gap-3">
                        <svg class="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        <span>{{ $siteSettings['about_accreditation_4'] }}</span>
                    </li>
                    @endif
                </ul>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 border border-slate-200 text-center">
                <h4 class="font-bold text-slate-900 mb-2">{{ $siteSettings['about_office_title'] ?? 'Visit Our Corporate Office' }}</h4>
                ...
                <div class="inline-block text-xs font-extrabold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-md border border-blue-200">
                    {{ $siteSettings['about_office_hours'] ?? 'Office Hours: Sat - Thu (09:00 AM - 06:00 PM)' }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mt-16 pt-16 border-t border-slate-200">
        <div class="text-center max-w-2xl mx-auto mb-12">
            <span class="text-xs font-bold uppercase tracking-wider text-blue-600 mb-1 block">{{ $siteSettings['about_leadership_subtitle'] ?? 'Board of Directors' }}</span>
            <h2 class="text-3xl font-extrabold text-slate-900">{{ $siteSettings['about_leadership_title'] ?? 'Our Leadership Team' }}</h2>
        </div>
